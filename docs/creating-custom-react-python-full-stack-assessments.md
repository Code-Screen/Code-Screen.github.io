# Creating Custom React + Python Assessments
...
You can then add the description of your assessment, choose the <strong>Full Stack</strong> category and then choose `React + Python` for the list of languages.
...
On the backend, all unit tests must use the `pytest` unit test framework, version `7.4.0`.

Your backend needs to be compatible with `Python` version `3.11`, and any dependencies need to be included in the `requirements.txt` file & must be available to download using the `pip` package installer.
...
CodeScreen uses GitHub Actions to run automated unit & integration tests. We provide the following GitHub Action file for React + Python assessments. 
...
jobs:
  ...
  e2eTests:
    ...
    - name: Check Flask backend server exists
      id: check_flask
    ...
    - name: Set up Python
      ...
      if: steps.check_cypress_tests.outputs.files_exists == 'true' && steps.check_flask.outputs.files_exists == 'true'
      with:
        python-version: '3.11'
