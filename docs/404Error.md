# GitHub 404 Error

When you create a custom test, or when a candidate submits their solution to one of your tests, you will be
granted access to the private repository on `GitHub` in the `codescreen` account.

When you click on the repository link URL, you may see the following:

<figure>
  <figcaption style="font-style: italic; font-weight: bold">GitHub 404 Error:</figcaption>
  </br>
  <img style="max-width: 60%;" src="404_error.png" alt="GitHub 404 Error"/>
</figure>

The reason for this is one of the following:

1. You are not currently logged into `GitHub`. <br>
    If this is the case, then please log into your `GitHub` account and try accessing the repo again.

2. You have not yet added your `GitHub` username to your profile on `CodeScreen`.  You can add your username <a href="https://app.codescreen.dev/#/client-account-edit" target="_blank">here</a>.<br>
    Once you add your username and save the changes, we will grant you access to all the custom test repos in your organiation's account.<br>
    We will also retroactively grant you access to all the repos of candidates who have submitted solutions to your tests.

3. You are logged in to `GitHub` under a different username than the one you have saved to your `CodeScreen` <a href="https://app.codescreen.dev/#/client-account-edit" target="_blank">profile</a>.<br>
    To solve this issue, please log into `GitHub` using the username you currently on have on your profile, or update your
    username on `CodeScreen`.
